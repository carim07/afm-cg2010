...
GLuint texid;
int texmode=1; // modo de aplicacion de textura
...
bool mipmap_ppm(char *ifile)
{
  ...
  for (int i=0;i<wt*ht;i++){
    ...
    imga[4*i+3]=(b-r>40&&b-g>10)? 0: 255;
  }
  delete[] img;
...
extern void drawEsfera(float radio, int nlatitud , int nlongitud);
...
//  if (cl_info) cout << "regen" << endl;
  ...
  glRotatef(amy,0,0,1); // rota los objetos alrededor de y
...
void initialize() {
  ...
  glutCreateWindow("Globo Terraqueo");// crea el main window //Cubo con textura
  ...
  // transparencias
  if (blend) {glEnable(GL_BLEND);glEnable(GL_ALPHA_TEST);}
