...
bool // variables de estado de este programa
  ...
  scissor=false,    // habilita el scissor test
...
GLuint texid;
int
  Sautomode=0; //modo de generacion de la textura (eye en regen)
...
static int ancho=16;
float ts=1,ts0,tf=0; // escala y fase de la textura
float splane[]={ts,1.f,0.f,tf}; // ts=modulo tf=distancia al origen

bool make_texture(){
  // primero y ultimo en 0 por el clamp; el ancho en 255
  static unsigned char img[tsize];
...
//  if (cl_info) cout << "regen" << endl;
  ...
  if (Sautomode==1) glTexGenfv(GL_S, GL_EYE_PLANE, splane);
...
void Reshape_cb(int width, int height){
  ...
  if (scissor) {
    Display_cb();
    glEnable(GL_SCISSOR_TEST);glScissor(w/4,h/4,w/2,h/2);
  }
}
...
void Motion_cb(int xm, int ym){ // drag
  if (boton==GLUT_LEFT_BUTTON){
    ...
    else if (modifiers==GLUT_ACTIVE_CTRL){ // cambio de escala de textura
      ts=ts0*exp((yclick-ym)/100.0);
      splane[0]=ts;
      glTexGenfv(GL_S, GL_OBJECT_PLANE, splane);
      cout << ts << "          \r"; cout.flush();
      if (Sautomode==1) regen();
      else Display_cb();
    }
    else if (modifiers==(GLUT_ACTIVE_ALT|GLUT_ACTIVE_CTRL)){ // fase
      if (!clamp) {
        // [0,1) porque se repite
        tf+=float(ym-yclick)/h; 
        tf-=int(tf); if (tf<0) tf+=1;
      }
      else{
        // los limites son +/-(zfar-znear)/2+1
        // es el radio de la BS, + 1 por el ancho
        float r=(zfar-znear)/2+1;
        tf+=2*r*(ym-yclick)/h;
        if (tf>r) tf-=2*r; else if (tf<-r) tf+=2*r;
      }
      yclick=ym;
      splane[3]=tf;
      glTexGenfv(GL_S, GL_OBJECT_PLANE, splane);
      cout << tf << "          \r"; cout.flush();
      if (Sautomode==1) regen();
      else Display_cb();
    }
    else { // manipulacion
...
void Mouse_cb(int button, int state, int x, int y){
  ...
  if (button==GLUT_LEFT_BUTTON){
    if (state==GLUT_DOWN) {
      ...
      else if (modifiers==GLUT_ACTIVE_CTRL) ts0=ts; // escala textura
...
void Keyboard_cb(unsigned char key,int x=0,int y=0) {
  switch (key){
    ...
    case 'j': case 'J': // luz fija a la camara o en el espacio
      ...
      regen();
      break;
    case 'k': case 'K': // scissor
      scissor=!scissor;
      if (scissor) {glEnable(GL_SCISSOR_TEST); glScissor(w/4,h/4,w/2,h/2);}
      else glDisable(GL_SCISSOR_TEST);
      if (cl_info) cout << ((scissor)? "Scissor" : "Sin Scissor") << endl;
      break;
    case 'l': case 'L': // wire
      wire=!wire;
...
void initialize() {
  ...
  glutCreateMenu(Menu_cb);
    ...
    glutAddMenuEntry("     [k]_Scissor ON/OFF        ", 'k');
