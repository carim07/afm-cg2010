...
int
  ...
  texmode=4, // modo de aplicacion de textura
...
void Keyboard_cb(unsigned char key,int x=0,int y=0) {
  switch (key){
    ...
    case 'n': case 'N':  // filtro GL_NEAREST/GL_LINEAR_MIPMAP_LINEAR
      ...
      if (cl_info) cout << ((nearest)? "Nearest" : "LinearMipmapLinear") << endl;
      break;
...
void initialize() {
  ...
  if (texmode<4){
    ...
    glDisable(GL_TEXTURE_2D);
  float splane[]={1.f,0.f,0.f,0.f},tplane[]={0.f,1.f,1.f,0.f};
