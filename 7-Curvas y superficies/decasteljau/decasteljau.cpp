...
float // colores
  ...
  p_c[]={.4f,0.f,0.f};        // punto de la curva
...
void dibuja(){
  ...
  // puntos de control
  glColor3fv(pc_c); glPointSize(5);
  glBegin(GL_POINTS);
  for (i=0;i<npc;i++) glVertex4fv(pc[i]);
  glEnd();
  // poligono de control
  glColor3fv(lc_c); glLineWidth(2);
  ...
  glColor3fv(la_c); glLineWidth(1); glPointSize(7);
  for (j=1;j<(lev<=npc-1?lev:npc-1);j++) {
    glBegin(GL_LINE_STRIP);
    for (i=0;i<npc-j;i++) glVertex4fv(pd[i].lerp(pd[i],pd[i+1],u));
    glEnd();
  }
  glColor3fv(p_c);glBegin(GL_POINTS);
  for (j=0;j<(lev>=npc?1:npc-lev);j++) {
...
void Keyboard_cb(unsigned char key,int x=0,int y=0) {
  if (key==27){ // escape => exit
    ...
    /*
    int maxlod=0; double d;
    for (int i=0;i<npc-1;i++){
      d=fabs(pc[i+1][0]-pc[i][0]); if (d>maxlod) maxlod=d;
      d=fabs(pc[i+1][1]-pc[i][1]); if (d>maxlod) maxlod=d;
    }
    if (lod>=maxlod) {lod=maxlod; return;}
    */
    lod++;
    if (cl_info) {
