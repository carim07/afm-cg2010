    ...
    for (j=0;j<nv;j++){
      ...
      if (!(m.find(Arista(n1,n2))!=m.end())){  // esto es para no contar dos veces la misma arista ;)
        ...
        if(p[n1].es_frontera){  // si el nodo es frontera (no tiene elemento vecino a la "derecha")
          ...
          cout<<"tamaÃ±o "<<tamanio<<" pc1 "<<pc1<<" "<<"pc2 "<<pc2<<endl;
          nuevo=(p[n1]+p[n2]+p[pc1]+p[pc2])/4; //nuevo nodo promedio de los nodos de la arista y los centroides de los elementos que comparten la arista   (acÃ¡ explota)
          cout<<"tamaÃ±o "<<tamanio<<" pc1 "<<pc1<<" "<<"pc2 "<<pc2<<endl;
        }
