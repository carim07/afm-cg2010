...
extern float face_color[],line_color[],control_color[],point_color[],fondo[];

//==========================================

static float rand01(){return float(rand())/(RAND_MAX+1);} // [0,1)
...
void drawObjects(int change=0){
  ...
  which=(which+change)%nobj;

  //face_color[0]=face_color[1]=face_color[2]=1;

  static const float original_color[3]={face_color[0],face_color[1],face_color[2]};
  static float rand_color[3];
  static bool primera_vez=true;
  if (change!=0||primera_vez) {
    primera_vez=false;
    // colores aleatorios
    rand_color[0]=rand01();rand_color[1]=rand01();rand_color[2]=rand01();
    // pasteles
/*    float m=(rand_color[0]+rand_color[1]+rand_color[2])/3;
    rand_color[0]=(rand_color[0]+3*m)/4;
    rand_color[1]=(rand_color[1]+3*m)/4;
    rand_color[2]=(rand_color[2]+3*m)/4;*/
  }
 // memcpy(face_color,rand_color,3*sizeof(float));
  switch (which){
    case 0:
      drawTeapot();
  ...
  memcpy(face_color,original_color,3*sizeof(float));
}
